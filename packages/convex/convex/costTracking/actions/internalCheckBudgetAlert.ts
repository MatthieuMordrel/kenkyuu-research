 ...
 */
export const internalCheckBudgetAlert = internalAction({
  args: {
  ...
  handler: async (ctx, args): Promise<void> => {
    ...
    const monthlyCost = await ctx.runQuery(
      internal.costTracking.queries.internalGetMonthlyCost
        .internalGetMonthlyCost,
      {}
    );
    ...
    await ctx.runMutation(
      internal.costTracking.mutations.internalMarkBudgetAlertSent
        .internalMarkBudgetAlertSent,
      {
        key: alertSentKey,
      }
    );
