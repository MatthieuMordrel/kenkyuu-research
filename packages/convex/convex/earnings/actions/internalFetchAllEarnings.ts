...
export const internalFetchAllEarnings = internalAction({
  args: {},
  handler: async (ctx) => {
    ...
    const stocks = await ctx.runQuery(
      internal.earnings.queries.internalListAllStocks.internalListAllStocks,
      {}
    );
      ...
      try {
        ...
        if (entries.length > 0) {
          await ctx.runMutation(
            internal.earnings.mutations.internalUpsertEarnings
              .internalUpsertEarnings,
            {
              stockId: stock._id,
              symbol: stock.ticker,
              entries,
