...
export const startResearch = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    const budget = await ctx.runQuery(
      internal.costTracking.queries.checkBudgetReachedInternal
        .checkBudgetReachedInternal,
      {}
    );
    if (budget.reached) {
    ...
    const activeJobs = await ctx.runQuery(
      internal.research.queries.getConcurrencyJobsInternal
        .getConcurrencyJobsInternal,
      {}
    );
    ...
    const attempts = await ctx.runMutation(
      internal.research.mutations.incrementAttempts.incrementAttempts,
      { id: args.jobId }
    );
